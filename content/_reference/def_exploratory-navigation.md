---
draft: false
enableToc: false
readTimeMM: false
explorerHide: true
title: Exploratory Navigation
signatureCode:
contentSection:
contentType:
growth_stage:
readerMode:
fm_level:
description: ""
socialDescription: ""
mmShort: A way of moving through a digital garden by following links, previews, backlinks, search, graph relationships, and side paths rather than relying on a single fixed route.
tags: []
date: 2026-06-11
last_reviewed: 2026-06-11
socialImage: ""
aliases: []
permalink:
journeys:
---
## Exploratory Navigation

**Exploratory navigation** is a way of moving through a digital garden by following curiosity, context, and connection instead of relying only on a single fixed path.

- The goal is not to make every person follow the same path.
- The goal is to make it easier for each to find a meaningful path without getting lost.

In MNKY Math, exploratory navigation includes links, hover previews, mobile previews, Search, backlinks, Graph View, side paths, and directed paths.

These tools help you move through the garden relationally.

Rather than asking only:  What comes next?

Exploratory navigation also considers:  **_What is this connected to?_**
  
You might start with an essay, follow a definition, preview a supporting concept, check backlinks, search for a recurring word, or use Graph View to see how ideas cluster.  

That movement is not random.  

**==It is a supported way of wandering.==** 

Exploratory navigation matters because MNKY Math is not only organized as a sequence of pages. It is organized as a growing set of relationships between systems, behavior, measurement, incentives, agency, participation, and outcomes. 
  
A single path can introduce the work.  
  
Exploratory navigation helps the garden stay open, adaptive, and alive.
